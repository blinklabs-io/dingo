...
type testPParamsProvider struct {
	pparams lcommon.ProtocolParameters
}

func (p testPParamsProvider) GetCurrentPParams() lcommon.ProtocolParameters {
	return p.pparams
}

func (p testPParamsProvider) ProtocolParamsForSlot(
	uint64,
) lcommon.ProtocolParameters {
	return p.pparams
}

type testChainTipProvider struct {
	tip ochainsync.Tip
}

func (p testChainTipProvider) Tip() ochainsync.Tip {
	return p.tip
}

type testEpochNonceProvider struct {
	epoch uint64
	nonce []byte
}

func (p testEpochNonceProvider) CurrentEpoch() uint64 {
	return p.epoch
}

func (p testEpochNonceProvider) EpochForSlot(uint64) (uint64, error) {
	return p.epoch, nil
}

func (p testEpochNonceProvider) EpochNonce(uint64) []byte {
	return p.nonce
}

func makeTestConwayTxCbor(t *testing.T, txID byte) []byte {
	t.Helper()
	txHash := make([]byte, 32)
	txHash[0] = txID
	...
	if err != nil {
		t.Fatalf("encode test transaction: %v", err)
	}
	if _, err := conway.NewConwayTransactionFromCbor(txCbor); err != nil {
		t.Fatalf("decode generated test transaction: %v", err)
	}
	return txCbor
}

// TestForgingMempoolAdapterBuildsBlockFromPendingTransactionCbor verifies the
// adapted mempool source feeds pending transaction CBOR into block construction.
func TestForgingMempoolAdapterBuildsBlockFromPendingTransactionCbor(t *testing.T) {
	vrf, kes, opcert := devnetCredPaths()
	creds := forging.NewPoolCredentials()
	if err := creds.LoadFromFiles(vrf, kes, opcert); err != nil {
		t.Fatalf("load pool credentials: %v", err)
	}

	txCbor := makeTestConwayTxCbor(t, 0x01)
	source := testMempoolTransactionSource{
		txs: []mempool.MempoolTransaction{
			{
				Hash: "tx1",
	...
	builder, err := forging.NewDefaultBlockBuilder(forging.BlockBuilderConfig{
		Logger:  slog.New(slog.NewJSONHandler(io.Discard, nil)),
		Mempool: &forgingMempoolAdapter{source: source},
		PParamsProvider: testPParamsProvider{
		...
		EpochNonce:  testEpochNonceProvider{epoch: 1, nonce: make([]byte, 32)},
		Credentials: creds,
	})
	if err != nil {
		t.Fatalf("create block builder: %v", err)
	}
	...
	if len(block.Transactions()) != 1 {
		t.Fatalf("expected block to include 1 transaction, got %d",
			len(block.Transactions()))
	}

	decodedBlock, err := conway.NewConwayBlockFromCbor(blockCbor)
	if err != nil {
		t.Fatalf("decode forged block: %v", err)
	}
	if len(decodedBlock.Transactions()) != 1 {
		t.Fatalf("expected decoded block to include 1 transaction, got %d",
