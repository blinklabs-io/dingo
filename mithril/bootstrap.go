...
func Bootstrap(
	...
	// Step 4: Extract archive (skip if already extracted)
	...
	if immutableDir != "" {
		...
		_, err = ExtractArchive(
			archivePath, extractDir, cfg.Logger,
		)
		if err != nil {
			return nil, fmt.Errorf(
	...
	if len(snapshot.AncillaryLocations) > 0 {
		...
		if hasLedgerFiles(candidateDir) {
			...
			var ancArchivePath string
			ancillaryDir, ancArchivePath, err = downloadAncillary(
				ctx, cfg, snapshot, downloadDir,
			)
			if err != nil {
			...
			ancillaryArchivePath = ancArchivePath
		}
	}
...
func downloadAncillary(
	...
	if _, extractErr := ExtractArchive(
		ancillaryPath, ancillaryDir, cfg.Logger,
	); extractErr != nil {
		return "", "", fmt.Errorf(
...
// hasFileRecursive checks if a directory or its immediate
// subdirectories contain a file with the given name.
func hasFileRecursive(dir string, name string) bool {
