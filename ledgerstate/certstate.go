...
func parseCertStateConway(
	...
	// Find the DState: largest map/indef-map element
	dIdx := -1
	dSize := 0
	for i, elem := range certState {
		if len(elem) == 0 {
	...
	for i, elem := range certState {
		...
		if isMap && (dIdx < 0 || len(elem) < dSize) {
			dreps, vErr := parseVState(elem)
			if vErr == nil && len(dreps) > 0 {
				result.DReps = dreps
				break
			}
		}
...
func parsePStateConway(data []byte) ([]ParsedPool, error) {
	...
	// Find the pool params map: the largest map element
	for _, elem := range ps {
		if len(elem) == 0 {
			continue
		}
		major := elem[0] >> 5
		isMap := major == 5 || elem[0] == 0xbf
		// Pool params maps are always significantly larger than
		// the other PState fields (counters, small arrays).
		const minPoolParamsMapSize = 100
		if isMap && len(elem) > minPoolParamsMapSize {
			// This is the pool params map
			return parseCertPoolParamsMap(elem)
		}
	}
...
func parseVState(data []byte) ([]ParsedDRep, error) {
	...
	// Parse DRep registrations (index 0) using
	// decodeMapEntries to handle indefinite-length maps.
	entries, err := decodeMapEntries(vs[0])
	if err != nil {
		return nil, fmt.Errorf(
...
func parseRational(data []byte) (uint64, uint64) {
	...
	var arr []uint64
	if _, err := cbor.Decode(data, &arr); err == nil && len(arr) >= 2 {
		return arr[0], arr[1]
	}
