...
// zeroBlake2b224 is a pre-computed zero hash used to check whether
// PaymentKeyHash/StakeKeyHash returned a meaningful value.
var zeroBlake2b224 = ledger.NewBlake2b224(nil)
...
func decodeTxInFromBytes(
	...
	// Fall back to binary format for other lengths
	if len(data) >= 34 {
		txHash := make([]byte, 32)
		copy(txHash, data[:32])
		idx := uint32(data[32]) | uint32(data[33])<<8
...
// loading the entire UTxO set into memory.
...
func extractAddressKeys(addr []byte, result *ParsedUTxO) {
	...
	case addrType <= 3 && len(addr) >= 57:
		...
		result.PaymentKey = addr[1:29]
		result.StakingKey = addr[29:57]
...
func parseCborTxOut(
	...
	if datumOutput, ok := txOut.(interface {
		DatumHash() *lcommon.Blake2b256
	}); ok {
		if dh := datumOutput.DatumHash(); dh != nil {
			result.DatumHash = dh.Bytes()
		}
	}

	if multiAssetOutput, ok := txOut.(interface {
...
func ParseUTxOsFromFile(
	...
	return ParseUTxOsStreaming(
		cbor.RawMessage(mapData), era, callback,
	)
}
