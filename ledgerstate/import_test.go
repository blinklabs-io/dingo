...
// Mithril snapshot, extracts the ledger state, and imports it into
// a temporary SQLite database. This is an integration test that
// requires network access and takes several minutes.
func TestImportLedgerStateFromMithril(t *testing.T) {
	...
	if os.Getenv("DINGO_INTEGRATION_TEST") == "" {
		t.Skip(
			"set DINGO_INTEGRATION_TEST=1 to run integration tests",
		)
	}
	...
	aggregatorURL, err := mithril.AggregatorURLForNetwork("preview")
	require.NoError(t, err, "getting aggregator URL")

	downloadDir := t.TempDir()

	result, err := mithril.Bootstrap(ctx, mithril.BootstrapConfig{
		Network:       "preview",
		AggregatorURL: aggregatorURL,
		DownloadDir:   downloadDir,
		Logger:        logger,
		OnProgress: func(p mithril.DownloadProgress) {
			if p.TotalBytes > 0 && int(p.Percent)%10 == 0 {
				t.Logf(
					"download: %.1f%% (%d / %d bytes)",
					p.Percent,
					p.BytesDownloaded,
					p.TotalBytes,
				)
	...
	t.Logf("extract dir: %s", result.ExtractDir)
	t.Logf("ancillary dir: %s", result.AncillaryDir)

	// Search for ledger state in ancillary dir first, then
	// extract dir
	...
	// UTxO data: either inline or via tvar file
	if state.UTxOTablePath == "" {
		...
		t.Logf(
			"UTxOs will be imported from tvar file: %s",
			state.UTxOTablePath,
		)
	}
	require.NotNil(
