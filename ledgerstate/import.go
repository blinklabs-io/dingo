...
func importPools(
	...
	for _, pool := range pools {
		...
		if pool.MarginDen > 0 {
			// #nosec G115
			r := new(big.Rat).SetFrac64(
				int64(pool.MarginNum),
				int64(pool.MarginDen),
			)
...
func importTip(cfg ImportConfig) error {
	tip := cfg.State.Tip
	if tip == nil {
		return errors.New("snapshot tip is nil")
	}
	...
	epochCount, err := generateAndSaveEpochs(cfg, txn)
	if err != nil {
...
func generateAndSaveEpochs(
	...
	// data. The setEpochIgnoreDup helper below handles
	// this by ignoring duplicate epoch_id errors.
	...
	if len(bounds) == 0 {
		...
		// #nosec G115
		eraDesc := eras.GetEraById(uint(cfg.State.EraIndex))
		if eraDesc != nil && eraDesc.EpochLengthFunc != nil &&
			cfg.CardanoNodeConfig != nil {
			sl, el, err := eraDesc.EpochLengthFunc(
		...
		epochStartSlot := cfg.State.Tip.Slot
		if lengthInSlots > 0 &&
...
func resolveEraParams(
	...
	return 0, uint(slotSpan / epochSpan)
}
...
// isEpochDuplicate returns true if the error is a UNIQUE
// constraint violation on the epoch table. This allows
// idempotent epoch generation when resuming a partially
// completed import. Handles SQLite, MySQL, and PostgreSQL
// duplicate key error formats.
func isEpochDuplicate(err error) bool {
	if err == nil {
		return false
	}
	msg := err.Error()
	return strings.Contains(
		msg, "UNIQUE constraint failed: epoch",
	) ||
