...
func FindLedgerStateFile(extractedDir string) (string, error) {
	...
	if len(utxoHDDirs) > 0 {
		slices.SortFunc(utxoHDDirs, func(a, b string) int {
...
func parseSnapshotData(data []byte) (*RawLedgerState, error) {
	...
	if boundsErr != nil {
		// Non-fatal: continue without era bounds. Epoch
		// generation will fall back to the single-epoch path.
		_ = boundsErr
		eraBounds = nil
	}
	...
	if nonceErr == nil && epochNonce != nil {
...
func parseCurrentEra(
	...
	// Loop to peel off all version layers.
	for len(shelleyState) >= 2 {
...
func parseEpochNonce(headerStateData []byte) ([]byte, error) {
	...
	// Peel version wrapper(s)
	for len(praosState) == 2 {
		var ver uint64
		if _, decErr := cbor.Decode(
			praosState[0], &ver,
		); decErr != nil {
