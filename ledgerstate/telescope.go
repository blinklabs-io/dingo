...
func extractEraBoundsNested(
	...
	case 0:
		...
		if len(items) < 2 {
			return nil, fmt.Errorf(
				"current era %s has %d items, expected at least 2",
	...
	case 1:
		...
		restBounds, err := extractEraBoundsNested(
			rest, eraIndex+1,
		)
		if err != nil {
			return nil, err
...
func navigateTelescopeNested(
	...
	case 0:
		...
		if len(items) < 2 {
			return 0, nil, fmt.Errorf(
				"current era %s has %d items, expected 2",
				EraName(eraIndex),
				len(items),
			)
		}
	...
	case 1:
		...
		return navigateTelescopeNested(rest, eraIndex+1)
	default:
		return 0, nil, fmt.Errorf(
			"unexpected telescope tag %d at era %s",
			tag,
