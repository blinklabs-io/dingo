...
- Top-level tests call `t.Parallel()` in most packages, including every one that dominates the suite; new tests there should too. Keep a test sequential — with a `// Not t.Parallel: ...` comment — when it reaches process-global state: swapping a package-level seam (`syncDir`, `cleanupConsumedUtxosInterval`, `deliveryStallWarnInterval`, `leiosPersistMaxQueueBytes`) or another package's variable (`ledger.Close*Timeout`), replacing a process-global (`slog.SetDefault`, `config.PublishConfig`), asserting a delta of a process-wide counter (`BlobOrphanCount`), or taking a process-wide measurement (`testing.AllocsPerRun`, `runtime.NumGoroutine`, `testing.Benchmark`, `goleak.VerifyNone`). A fixture that serializes for the whole test instead — `database/lifecycle`'s `setFakeCloudBackingDir` holds `fakeCloudFixtureMu` until `t.Cleanup` — is parallel-safe. `internal/settingsresolve` and `bark/database_cloud_test.go` stay fully sequential: their fixtures rewrite a process-global (`globalConfig`, `barkFakeCloudDir`) with no such gate.
...
- Plugins (`database/plugin/`): blob = `badger` (default) | `gcs` | `s3`; metadata = `sqlite` (default) | `mysql` | `postgres`. Mempool = `fifo` (default) | `dag`; API = `blockfrost` | `mesh` | `utxorpc`. The non-default storage providers build only under `-tags dingo_extra_plugins`.
- Metadata storage is typed `database/sql` generated by sqlc from `sqlc.yaml`, not an ORM. Regenerate with `make sql` after changing queries.
