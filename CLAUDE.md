...
- CI also runs both DevNet suites (`.github/workflows/devnet.yml`) on pull requests touching a consensus-sensitive path (`.github/devnet-paths.txt`), and unconditionally on pushes to `main`, `v*` tags, and nightly. Stable check name `devnet-gate`; not yet required because the suites are intermittently red on `main` from #3029 (see `docs/BRANCH_PROTECTION.md`). New consensus-sensitive packages need a pattern in `.github/devnet-paths.txt` — `internal/test/ci/` fails when a pattern matches no real file. This does not replace running DevNet locally for consensus-affecting work.
- `DEVNET_ARTIFACT_DIR=<dir>` makes `run-tests.sh` collect node logs, generated genesis/config, container status, and the `txpump` log before teardown.
