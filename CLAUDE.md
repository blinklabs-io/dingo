...
make docs-parity
make config-parity  # embedded network configs vs docker-cardano-configs; needs network
make golines
...
- Metadata storage is typed `database/sql` generated by sqlc from `sqlc.yaml`, not an ORM. Regenerate with `make sql`; `make gorm-check` fails if the removed ORM returns.
