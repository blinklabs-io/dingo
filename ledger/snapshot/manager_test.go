...
func TestHandleEpochTransitionPersistsRewardStateInputs(t *testing.T) {
	...
	copy(otherPoolID[:], []byte("other_12345678901234567890AB"))
	require.NoError(t, db.UpdatePoolOpCertSequence(poolID, 1, 600, nil))
	require.NoError(t, db.UpdatePoolOpCertSequence(otherPoolID, 1, 700, nil))
	require.NoError(t, db.UpdatePoolOpCertSequence(poolID, 2, 432000, nil))
	...
	require.Nil(t, inputs[0].BlocksProduced)
	require.Nil(t, inputs[0].TotalBlocksInEpoch)
