...
func (ls *LedgerState) calculateStakeRewardApplication(
	...
	epochs, ok := stakeRewardEpochsForNewEpoch(newEpoch)
	if !ok {
...
func (ls *LedgerState) handleRewardPrecomputeEpochTransition(evt event.Event) {
	...
	if !ls.rewardPrecomputeBusy.CompareAndSwap(false, true) {
		ls.config.Logger.Debug(
			"skipping reward precompute: previous run still active",
			"component", "ledger",
			"new_epoch", epochEvent.NewEpoch,
		)
		return
	}
	defer ls.rewardPrecomputeBusy.Store(false)

	if err := ls.precomputeStakeRewardsAfterEpochTransition(epochEvent); err != nil {
...
func (ls *LedgerState) precomputeStakeRewardsAfterEpochTransition(
	...
	return writeTxn.Do(func(txn *database.Txn) error {
		...
		if !guardOK {
			ls.config.Logger.Debug(
				"dropping precomputed stake rewards: reward snapshot changed since calculation",
				"component", "ledger",
				"reward_snapshot_epoch", app.epochs.snapshot,
				"application_epoch", newEpoch,
				"captured_slot", capturedSlot,
...
// stakeRewardPrecomputeSnapshotGuardOK reports whether the reward_snapshot
// row an already-computed stakeRewardApplication consumed is still the
// current row for its epoch. It re-reads the row inside the caller's write
// transaction and compares captured/boundary slots against the values
// recorded when the calculation ran (see
// stakeRewardApplication.snapshotCapturedSlot/snapshotBoundarySlot). A
// mismatch, or a missing row, means the snapshot moved between the read and
// write phases -- most commonly a rollback replaced or removed it -- and the
// computed application must be dropped rather than persisted; the
// synchronous boundary-application path recomputes authoritatively when it
// runs.
