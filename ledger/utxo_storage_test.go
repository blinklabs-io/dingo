	...
	for _, deleteTxBlob := range []bool{false, true} {
		t.Run(fmt.Sprintf("delete_tx_blob=%t", deleteTxBlob), func(t *testing.T) {
			...
			err = lookupTxn.Do(func(txn *database.Txn) error {
				...
				// The recovery path should heal the missing blob so future lookups
				// do not need to re-derive it from the producer block.
				repaired, err := db.Blob().GetUtxo(txn.Blob(), txId, outputIdx)
				require.NoError(t, err)
				require.Equal(t, expectedUtxo.Output.Cbor(), repaired)
				return nil
			})
			require.NoError(t, err)
		})
