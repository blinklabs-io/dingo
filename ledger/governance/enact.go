...
// applyTreasuryWithdrawal moves lovelace from the treasury to the
// reward accounts listed in the proposal. Missing or stale network
// state is treated as a zero starting balance, which makes the update
// a no-op on treasury but still credits the recipients' reward
// accounts.
func applyTreasuryWithdrawal(
	ctx *EnactmentContext,
	a *lcommon.TreasuryWithdrawalGovAction,
) error {
	state, err := ctx.DB.Metadata().GetNetworkState(ctx.Txn.Metadata())
	if err != nil {
	...
	if total > treasury {
		// Clamp instead of failing: without full ada-pots tracking we
		// would otherwise reject withdrawals on a fresh import even
		// when they are on-chain valid. The reserve of accuracy here
		// lives in ada-pots work, tracked separately.
		total = treasury
	}
...
// action's credential-to-epoch map.
