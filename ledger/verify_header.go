...
func (ls *LedgerState) ensureEpochForSlot(
	...
	const maxAdvance = 100 // Safety limit against runaway loops
	var prevTailEpoch uint64
	for i := range maxAdvance {
		// Snapshot the current cache tail to detect stalls
		ls.RLock()
		cacheLen := len(ls.epochCache)
		var tailEpoch uint64
		if cacheLen > 0 {
			tailEpoch = ls.epochCache[cacheLen-1].EpochId
		}
		ls.RUnlock()
		if cacheLen == 0 {
			return models.Epoch{}, errors.New("epoch cache is empty")
		...
		if i > 0 && tailEpoch == prevTailEpoch {
			// advanceEpochCache returned nil but made no progress
			return models.Epoch{}, fmt.Errorf(
				"epoch cache stalled at epoch %d, cannot reach slot %d",
				tailEpoch,
				targetSlot,
			)
		}
		prevTailEpoch = tailEpoch
...
func (ls *LedgerState) advanceEpochCache() error {
	...
	// Update cache under write lock, re-validating the base epoch
	ls.Lock()
	...
	// Verify that the snapshot we computed from is still the cache tail.
	// A concurrent processEpochRollover may have replaced the cache with
	// different epoch parameters (e.g. after a hard fork).
	if lastCached.EpochId != lastEpoch.EpochId ||
		...
		// Cache was replaced since our snapshot; discard stale result
		ls.Unlock()
		return nil
	}
