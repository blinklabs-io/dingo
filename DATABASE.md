...
true; `IsTxCborPartsStorage` applies the same check so it stays a reliable
predictor of whether decode will succeed. `ReassembleTxCbor`'s
offset/length bounds checks compare the offset against the block length
before subtracting (`offset > blockLen || length > blockLen-offset`), which
cannot underflow, rather than adding `offset+length` and comparing against
the block length, which could overflow `uint32` and wrap past the check.

S3 and GCS `GetBlock` re-derive the block's identity from the returned bytes
(`blockverify.Hash`, `database/plugin/blob/internal/blockverify`) before
handing them to a caller: it decodes the bytes using the type recorded in
`bp..._metadata` and compares the result's own hash against the requested
`bp` key's hash, since neither backend offers a content-addressing guarantee
of its own — corruption, an eventual-consistency stale read, or a
misdirected request could otherwise return bytes for a different block than
the one asked for. Bark's archive fetch already re-verifies its downloaded
blocks the same way (`verifyArchiveBlock`); this closes the equivalent gap
for the two cloud object-store blob backends. Badger's local `GetBlock`
...
materialize an unbounded result. Each SQL chunk is queried with `LIMIT
maxResults+1` so an over-budget result is detected rather than silently
truncated into a partial (and therefore wrong) answer; exceeding the bound
returns `models.ErrTooManyUtxoResults`. Callers with no more specific limit
of their own use `database.MaxUtxosByAddressResults`. This is unrelated to
