...
| `move_instantaneous_rewards_reward` | `id`, `mir_id`, `credential_tag`, `credential`, `amount` | PK `id`; indexes `mir_id`, `(credential_tag, credential)` | MIR reward rows. `credential_tag`: 0 key hash, 1 script hash. Join `mir_id -> move_instantaneous_rewards.id`. |
