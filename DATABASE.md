...
| Table | Columns | Keys / indexes | Relationships and notes |
|---|---|---|---|
| `transaction` | `id`, `hash`, `block_hash`, `slot`, `block_index`, `type`, `fee`, `collateral_fee`, `ttl`, `valid`, `metadata` | PK `id`; unique `hash`; indexes `block_hash`, `slot` | One row per transaction. `block_hash` and `slot` point to the blob block. `fee` is the declared body fee; `collateral_fee` is the collateral consumed into the fee pot by a phase-2-invalid transaction (collateral inputs minus collateral return, per the Alonzo/Babbage UTXOS rule) and zero for valid transactions. The reward fee pot sums `fee` for valid rows plus `collateral_fee` for invalid rows; rows written before the column existed are NULL and excluded. `metadata` is populated only in API mode. |
...
| Table | Columns | Keys / indexes | Relationships and notes |
|---|---|---|---|
| `account` | `id`, `staking_key`, `credential_tag`, `pool`, `drep`, `added_slot`, `created_slot`, `certificate_id`, `reward`, `drep_type`, `active` | PK `id`; unique `(credential_tag, staking_key)`; indexes pool/DRep/active lookup combinations, including leftmost `active` coverage for reconcile scans | Current stake account state. `credential_tag`: 0 key hash, 1 script hash. Historical changes are in certificate-specific tables. Deregistration clears the active delegation state; a later plain registration does not reuse older `pool`/`drep` values. `drep_type`: 0 key hash, 1 script hash, 2 AlwaysAbstain, 3 AlwaysNoConfidence. `created_slot` is the immutable slot the row was first created (0 for Shelley-genesis delegated accounts); unlike `added_slot` it is never bumped by later delegation/registration changes. It is stamped at account creation (via the `AccountCreatedSlotUnset` sentinel resolved in `saveAccount`, including the phantom row synthesized when a deregistration certificate references a never-registered credential, which records the certificate slot rather than 0) and, for databases predating the column, backfilled once from the earliest registration certificate (or 0 when none) by `BackfillAccountCreatedSlot`. That backfill is gated on the durable `backfill_checkpoint` phase `account_created_slot` (crash-safe, runs its scan once) and, on MySQL/Postgres, stamps with one `CASE` UPDATE per chunk to collapse round-trips while SQLite keeps per-slot grouped UPDATEs. Slot-active account lookups use certificate history: credentials with a registration certificate are active unless a later deregistration supersedes it, and credentials with no registration certificate, such as Shelley-genesis delegated accounts, are active from chain start and deactivated only by a deregistration certificate. The point-in-time decision uses `created_slot` (did the account exist as of the slot) plus deregistration history, never the mutable `active`/`added_slot` columns, because a later re-delegation bumps `added_slot` forward without changing activity. |
...
| `pool_opcert_sequence` | `id`, `pool_key_hash`, `slot`, `sequence` | PK `id`; unique `(pool_key_hash, slot)`; index `slot` | Observed operational certificate sequence by slot. Read before write inside the block-apply transaction to enforce inbound opcert counter monotonicity; per-slot rows let rollback drop entries past the rollback slot and recompute `pool.latest_op_cert_sequence`. Reward block-performance counts read these raw issuer rows but exclude TPraos overlay slots when decentralization is non-zero, matching cardano-ledger `incrBlocks`. |
...
| `reward_live_stake` | `id`, `credential_tag`, `staking_key`, `pool_key_hash`, `utxo_stake`, `reward_stake`, `total_stake`, `registered`, `pool_delegation_slot`, `pool_delegation_block_index`, `pool_delegation_cert_index`, `updated_slot` | PK `id`; unique `(credential_tag, staking_key)`; index `(pool_key_hash, credential_tag, total_stake, registered, pool_delegation_slot)` | Live per-stake-credential reward aggregate. Metadata writes refresh touched credentials when UTxOs, account registration/delegation, and reward balances change; Mithril import rebuilds it after cert-state import and stale-state reconcile from the union of account credentials and live UTxO stake credentials. Non-empty source stake credentials must be 28-byte key/script credentials (`credential_tag` 0 or 1); malformed account/live-UTxO rows fail refresh or rebuild before entering the aggregate. Credentials with UTxO stake but no active account remain as `registered=false` rows with no pool and are ignored by reward snapshot input copying until a live registered delegation exists. The `pool_delegation_*` columns store the certificate order for the current pool delegation for rollback/rebuild fidelity; reward snapshot capture preserves that live delegation across later pool parameter registrations and filters by active pool input, registered account state, and positive total stake. Epoch-boundary reward snapshots copy from this table instead of joining the full live UTxO set, which is required to keep mainnet rollover from starving block production near boundary leader slots. `MetadataStore.RewardLiveStakeNeedsBackfill` reports whether this table needs a `RebuildRewardLiveStake` pass: true only when `account` has rows but this table is completely empty (an upgraded database that synced before this table existed, so only post-upgrade credentials got incremental rows), false on both a legitimately fresh database (both empty) and an already-populated one. Node startup (`node.go`, the normal `Run` path) calls this once before resuming chain following and runs a one-time `RebuildRewardLiveStake` at the current ledger tip slot when it reports true, so an upgraded database cannot hand the next epoch-boundary mark snapshot a near-empty aggregate (#1959). |
| `reward_ada_pots` | `id`, `epoch`, `treasury`, `reserves`, `fees`, `rewards`, `captured_slot` | PK `id`; unique `epoch`; index `captured_slot` | Reward ADA pots captured at an epoch boundary after boundary mutations; later delayed reward calculation uses these pots and updates `rewards` with the calculated reward pot. When rollback removes reward output rows for snapshot epoch `S`, the derived `rewards` value for pot epoch `S+2` is reset so replay recomputes it from the surviving inputs. |
| `reward_snapshot` | `id`, `epoch`, `snapshot_type`, `total_active_stake`, `total_pool_count`, `total_delegators`, `captured_slot`, `boundary_slot`, `epoch_nonce`, `protocol_version` | PK `id`; unique `(epoch, snapshot_type)`; indexes `captured_slot`, `boundary_slot` | Reward-calculation snapshot metadata. Stake rewards applied at epoch N use the Mark snapshot from epoch N-3, block production from N-2, and ADA pots plus reward protocol parameters from the RUPD calculation epoch N-1. Historical post-Mithril `pool_stake_snapshot` seed rows may intentionally have no matching `reward_snapshot`; reward application skips missing input bundles rather than using current live stake as historical data. |
| `reward_pool_input` | `id`, `epoch`, `pool_key_hash`, `reward_account`, `reward_account_credential_tag`, `pledge`, `delegated_stake`, `owner_stake`, `cost`, `margin`, `delegator_count`, `blocks_produced`, `total_blocks_in_epoch`, `captured_slot`, `boundary_slot` | PK `id`; unique `(epoch, pool_key_hash)`; indexes `captured_slot`, `boundary_slot` | Per-pool reward inputs. Snapshot capture freezes stake and pool-parameter inputs only; block-performance counts are recomputed from `pool_opcert_sequence` for the later reward performance epoch so TPraos overlay filtering and delayed epoch selection match cardano-ledger. Reward input capture requires a matching pool registration at the snapshot slot, a 28-byte key/script reward account credential, 28-byte pool owner key hashes, and a non-null pool margin; a pool that fails any of these checks (missing/legacy/corrupt registration data, e.g. from a partial import or Mithril bootstrap gap) is excluded, with a warning, from `reward_pool_input`/`reward_stake_input` and from the owning `reward_snapshot`'s totals for that epoch, rather than failing capture for every pool. `pool_stake_snapshot`/`epoch_summary`, captured from the unfiltered stake distribution earlier in the same snapshot write, still include that pool's real observed stake. Reward calculation and precomputed-output admission require each pool input's `captured_slot` and `boundary_slot` to match the owning `reward_snapshot`, require a valid reward account, reject margins outside `[0,1]`, and reject owner stake above delegated stake so malformed persisted rows cannot silently become zero-margin pools or skew owner/member rewards. Pre-Babbage reward prefilter eligibility is resolved from stake-account certificate history immediately before the first reward-update slot (`epoch.start + 4k/f + 1`) rather than stored on this snapshot row; final spendability is still checked against the live account set when rewards are applied. Snapshot refreshes call `DeleteRewardInputsForEpoch` before saving the fresh pool/stake input bundle and `DeleteRewardOutputsForEpoch` to invalidate stale precomputed outputs, so old rows for disappeared pools cannot survive. Logical join to `pool.pool_key_hash`. Rows are absent for historical post-Mithril seed snapshots whose exact reward inputs have not been backfilled. |
| `reward_stake_input` | `id`, `epoch`, `pool_key_hash`, `credential_tag`, `staking_key`, `stake`, `owner`, `registered`, `captured_slot`, `boundary_slot` | PK `id`; unique `(epoch, pool_key_hash, credential_tag, staking_key)`; indexes `captured_slot`, `boundary_slot` | Frozen positive-stake per-credential reward snapshot input copied from `reward_live_stake`, with owner flags for pledge and member-reward exclusion. Pool owner matching is key-hash only (`credential_tag = 0`); a script credential with the same 28-byte hash is still member stake. Capture requires a 28-byte stake credential hash and key/script credential tag; a pool with a malformed owner key hash on its own registration is excluded (see `reward_pool_input`) before owner stake would otherwise be derived from it, so its delegators are simply absent from this table for the epoch rather than blocking capture for other pools. The stored `registered` value reflects the snapshot aggregate state; pre-Babbage reward prefilter eligibility is resolved from certificate history at reward calculation time. Reward input capture and later recalculation require the pool input count to match `reward_snapshot.total_pool_count`, require stake input `captured_slot` and `boundary_slot` to match the owning snapshot, require per-credential stake rows to sum exactly to each matching `reward_pool_input.delegated_stake`, require owner-marked rows to sum exactly to `reward_pool_input.owner_stake`, require per-pool row counts to match `reward_pool_input.delegator_count`, and reject rows for unknown pools, so a stale live aggregate or malformed persisted bundle cannot silently skew owner/member rewards or turn member rewards into reserve-returned undistributed rewards. Snapshot refreshes call `DeleteRewardInputsForEpoch` before saving the fresh pool/stake input bundle and `DeleteRewardOutputsForEpoch` to invalidate stale precomputed outputs, so old credential/output rows cannot survive if delegation or stake disappears. Rows are absent for historical post-Mithril seed snapshots whose exact reward inputs have not been backfilled. |
| `reward_pool_output` | `id`, `epoch`, `pool_key_hash`, `apparent_performance`, `optimal_reward`, `total_reward`, `leader_reward`, `member_reward_total`, `owner_stake`, `undistributed`, `unspendable`, `captured_slot`, `boundary_slot` | PK `id`; unique `(epoch, pool_key_hash)`; indexes `captured_slot`, `boundary_slot` | Per-pool reward calculation output. `epoch` is the reward snapshot epoch; the external earned epoch is `epoch + 1` and spendable epoch is `epoch + 3`. `captured_slot` is when the calculation output was produced; `boundary_slot` is when it is applied. `leader_reward` is the formula's operator reward and can be non-zero even when pre-Babbage filtering produces no delivered leader account output; Babbage+ precomputed rows must include the matching delivered leader output when `leader_reward` is non-zero. `member_reward_total` is the delivered member total. Reward calculation/precompute replaces all output rows for the snapshot epoch before saving the fresh pool/account output set, so a recalculation that produces fewer rows cannot leave stale pool outputs behind. Boundary application only trusts precomputed pool outputs when the frozen `reward_pool_input` rows are structurally valid, their captured/boundary slots match `reward_snapshot`, their row count still matches `reward_snapshot.total_pool_count`, their delegated stake total still matches `reward_snapshot.total_active_stake`, their delegator count total still matches `reward_snapshot.total_delegators`, their owner stake matches the corresponding pool output, their pool-key set exactly matches that input bundle, output `boundary_slot` equals the application boundary, output `captured_slot` is not after that boundary, the matching `reward_ada_pots.rewards` value is at least fees and does not imply incentives above reserves, delivered account rows match the per-pool totals, leader account rows target the frozen pool reward account, and malformed zero-value rows are absent for the same snapshot epoch, so orphaned or malformed output rows cannot apply without the frozen input evidence and possible reward pot they were derived from. |
| `reward_account_output` | `id`, `epoch`, `credential_tag`, `staking_key`, `pool_key_hash`, `reward_type`, `amount`, `spendable`, `captured_slot`, `boundary_slot` | PK `id`; unique `(epoch, credential_tag, staking_key, pool_key_hash, reward_type)`; indexes `captured_slot`, `boundary_slot` | Per-account reward calculation output. `epoch` is the reward snapshot epoch; the external earned epoch is `epoch + 1` and spendable epoch is `epoch + 3`. `spendable=false` records rewards routed away from a deregistered reward account; the final spendability check uses the full `(credential_tag, staking_key)` identity so same-hash key and script credentials do not merge. Leader rows are admitted only when their `(credential_tag, staking_key)` identity matches the corresponding frozen pool reward account. `captured_slot` can precede `boundary_slot` when the ledger precomputes rewards after the prior epoch transition; for pre-Babbage rows it must not precede the first RUPD prefilter slot because account registration in that slot is not visible to the beginning-of-slot reward calculation. Precomputed application requires account output `boundary_slot` to equal the application boundary and `captured_slot` to be no later than that boundary. Reward calculation/precompute replaces all output rows for the snapshot epoch before saving fresh outputs, so stale per-account rewards cannot survive if stake, pool, or registration state changes before recalculation. |
...
imported/bootstrap data, sums only UTxOs live at that slot, and counts every
