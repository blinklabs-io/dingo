...
used by the new store. Release `v2alpha1` (integer version 2) adds the
`node_settings_gate` table described below; it applies on top of `v1alpha1`
for both fresh and existing databases and has no backfill.
...
- **postgres/mysql metadata**: no client/server database has an equivalent to SQLite's file-level `VACUUM INTO`. This needs either shelling out to `pg_dump`/`mysqldump` (external binary dependency, version-compatibility surface) or a hand-rolled dump/restore across the metadata schema (~93 tables) with its own consistent-snapshot-isolation and FK-safe-restore-ordering design — a substantially larger undertaking than the blob-store case.
...
against a wrong default. `StorageMode`, `Network`, and `StartEra` are always
included, since their gates are `LatchEnum`/`FrozenFillOnce` classes that
already treat their own zero value ("" for `StartEra`) as "not known on
this path" rather than a real configuration.
