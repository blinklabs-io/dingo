...
it from the existing `account` rows whose `created_slot` is 0. Like `v2` and
`v3` it ships only a SQLite `expand.sql`. `staking_key` is a `blob` inside the
composite primary key, so MySQL needs the same `(255)` prefix the registry
derives from the `CREATE TABLE` in this migration. Both statements are
re-runnable after an interrupted upgrade: the table uses
...
`SELECT`'s `FROM` clause. The backfill scans `account` once with no index on
`created_slot`; on an imported database most rows match, so a scan is the
access path either way.
...
later rollback restore the account this reconcile just tombstoned. This uses
