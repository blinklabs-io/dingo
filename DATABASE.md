...
| `reward_live_stake` | `id`, `credential_tag`, `staking_key`, `pool_key_hash`, `utxo_stake`, `reward_stake`, `total_stake`, `registered`, `pool_delegation_slot`, `pool_delegation_block_index`, `pool_delegation_cert_index`, `updated_slot` | PK `id`; unique `(credential_tag, staking_key)` | Live per-stake-credential aggregate maintained transactionally with UTxO, account, delegation, and reward-balance writes. The `pool_delegation_*` columns record the certificate order used to derive the current pool for rollback/rebuild bookkeeping; no snapshot eligibility rule currently consumes them. Non-empty source credentials must be 28-byte key/script credentials. UTxO-only credentials remain unregistered and undelegated. The current Mark snapshot path still performs its slot-aware UTxO join; this table is groundwork for a future reward-snapshot consumer. Node startup checks for missing aggregate credentials after recovery and rebuilds from canonical account/live-UTxO state at the persisted tip when necessary. Mithril import does not rebuild it directly; normal node startup does. |
...
| `reward_snapshot` | `id`, `epoch`, `snapshot_type`, `total_active_stake`, `total_pool_count`, `total_delegators`, `captured_slot`, `boundary_slot`, `epoch_nonce`, `protocol_version` | PK `id`; unique `(epoch, snapshot_type)`; indexes `captured_slot`, `boundary_slot` | Reward snapshot metadata recorded by the epoch rotation path. |
| `reward_pool_input` | `id`, `epoch`, `pool_key_hash`, `reward_account`, `reward_account_credential_tag`, `pledge`, `delegated_stake`, `owner_stake`, `cost`, `margin`, `delegator_count`, `blocks_produced`, `total_blocks_in_epoch`, `captured_slot`, `boundary_slot` | PK `id`; unique `(epoch, pool_key_hash)`; indexes `captured_slot`, `boundary_slot` | Per-pool metadata captured by epoch rotation. Stake totals come from the existing Mark distribution; pool parameters are selected as effective during the ended epoch. Block counts are stored on the row at capture time. Missing registrations are logged and leave registration-derived fields at zero or null. No reward calculation/application admission policy currently consumes these rows. Logical join to `pool.pool_key_hash`. |
