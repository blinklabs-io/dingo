...
first acquires lifetime admission for a coordinated snapshot and reserves a
metadata read-pool connection (`types.ReadReserver`, implemented by
`sqlstore.Store.ReserveRead`), then acquires
...
inside the barrier. `ReadSnapshotLimit` caps these snapshots at one fewer than
the metadata pool size, and admission remains held until the combined
transaction ends. Client-paced streams therefore cannot occupy the connection
needed by operational reads within a destructive transition. Every combined
write transaction holds the commit barrier's shared side for its whole
lifetime. Multi-transaction primary-chain rollbacks hold a separate
destructive-transition barrier from their blob-only block deletes through the
later ledger metadata truncation; `PauseCommitsContext` takes the
destructive-transition barrier's shared side before the commit barrier, so it
cannot open between the blob-only deletion and the metadata truncation. Both
views therefore describe one logical database boundary and remain fixed for
the lifetime of the streamed query; commits are paused only for construction,
not while results are streamed.
