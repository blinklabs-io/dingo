...
| `pparams` | `id`, `cbor`, `added_slot`, `epoch`, `era_id` | PK `id`; index `added_slot` | CBOR protocol parameters. Query by `epoch <= ?` and matching `era_id`. Dijkstra's on-chain CBOR intentionally omits the genesis-only `CommitteeStakeCoverage` and `QuorumStakeThreshold` fields. Ledger reconstruction rehydrates any absent values from the configured Dijkstra genesis through `loadPersistedProtocolParameters` and validates the reconstructed pair before publishing it; an invalid configured pair therefore fails restart instead of entering consensus or Leios state. A Mithril ledger-state import writes the snapshot epoch's current parameters and any distinct previous parameters compatible with the preceding epoch's actual era in one metadata transaction, while reusing an already-satisfying row on re-entry. A translated new-era previous payload is never stored under the old era; the dependent imported reward basis and any stale provisional inputs are removed instead. A hard fork whose `HardForkFunc` fabricates a PlutusV2 cost model default (`HardForkBabbage`, needed because internal script validation must have one before any real update introduces it -- see `synthetic_v2_cost_model` below) never persists that fabricated value here: `transitionToEraFrom` strips it (`withoutSyntheticV2CostModel`) before encoding the row, so a historical/reporting read of the row at that epoch matches what the chain had actually enacted rather than dating the cost model to the era transition (blinklabs-io/dingo#4127). `LedgerState.currentPParams` still carries the fabricated value in memory, restored (`withDefaultV2CostModelIfMissing`) both on load and after a rollback reloads the row, so internal validation is unaffected and does not depend on whether the process restarted or rolled back. Rows written by earlier releases keep the fabricated value; only a resync rewrites them. |
