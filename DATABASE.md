...
| `account_reward_delta` | `id`, `staking_key`, `credential_tag`, `tx_hash`, `amount`, `previous_reward`, `reconciled_amount`, `added_slot`, `withdrawal` | PK `id`; indexes `(credential_tag, staking_key)`, `tx_hash`, `added_slot`, `withdrawal`; unique `(withdrawal, tx_hash, credential_tag, staking_key, added_slot)` | Rollback-aware reward-account change journal. `tx_hash` is non-null; credit writers use an empty blob only when no event discriminator exists. Credit rows add `amount`; positive withdrawal rows subtract `amount` from `account.reward`, store the pre-withdrawal balance in `previous_reward`, and use `tx_hash` plus the full stake credential identity as their slot-independent logical replay key. The physical unique key also includes `added_slot` so distinct per-epoch credits remain separate while same-boundary credit re-ingest is idempotent. Governance proposal credits store a 32-byte proposal-event discriminator derived from proposal `tx_hash` plus `action_index`, not the raw proposal transaction hash alone. `reconciled_amount` is set only by `ReconcileAccountRewardBalance` (a narrow, explicitly opt-in reward-balance correction primitive; not currently wired to any caller in this codebase -- see dingo#4560's review discussion), which overwrites `account.reward` to a corrected value and journals the pre-overwrite balance as a synthetic withdrawal row so `DeleteAccountRewardsAfterSlot` can invert it; `previous_reward` on that row still holds the pre-overwrite balance for that inversion, but a historical-reconstruction reader (`historicalRewardsBatch`/`historicalRewardsAtBoundary`) that walks back through the row for a boundary slot before the correction resolves to `reconciled_amount` instead, since the correction proved `previous_reward` wrong rather than merely superseded. NULL on every ordinary credit or real withdrawal row. Logical join to `account.(credential_tag, staking_key)`. This join is not enforced, so `DeleteAccountRewardsAfterSlot` treats a row whose account is missing as a no-op rather than an error: the balance it journals lives on the account row, so with that row gone there is nothing to restore. It logs a warning, drops the stale journal row, and refreshes the credential's `reward_live_stake` aggregate. Failing instead would abort the whole rollback, and because startup reconciliation rolls the ledger back to the chain tip through this path, a database holding even one such orphan could never finish booting. |
...
WHERE withdrawal = true AND reconciled_amount IS NULL
  AND credential_tag = $1 AND staking_key = decode($2, 'hex');
...
their sign. `reconciled_amount IS NULL` excludes a `ReconcileAccountRewardBalance`
correction's withdrawal-shaped row (see the `account_reward_delta` row above):
its `amount` is the pre-correction balance the correction proved wrong, not
coin any delegator withdrew, so it must not inflate `withdrawals_sum`.
