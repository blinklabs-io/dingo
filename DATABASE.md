...
forward into a later renewal of the same membership rather than being cleared
by it, matching cardano-ledger, where committee state is keyed by cold
credential with no term component and an enacted `UpdateCommittee` retains the
existing entry for a member that is still in the committee it enacts
(`Map.intersection` in the Conway EPOCH rule). A resigned member cannot
...
**Mithril reward-state repair.** Migration `v22` sets `sync_state.mithril_reward_repair_pending` when a legacy Mithril ledger anchor has a `reward_ada_pots` row with no imported fee basis. Its presence blocks normal serving and dispatches certified v2 catch-up. The catch-up verifies the existing chain before reconciling ledger rows; API storage resets `backfill_checkpoint.metadata` to slot 0 and rebuilds historical metadata through the certified anchor. Successful completion clears the repair marker only after backfill and critical-index rebuilding; an interrupted run retains the marker and sync status so startup remains blocked while the repair resumes.
