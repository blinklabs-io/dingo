...
| `asset` | `id`, `utxo_id`, `policy_id`, `name`, `name_hex`, `fingerprint`, `amount` | PK `id`; unique `(name, policy_id, utxo_id)`; indexes `policy_id`, `name_hex`, `fingerprint`, `amount` | Multi-asset quantities attached to `utxo.id`. Use `utxo.deleted_slot = 0` for live balances. |
