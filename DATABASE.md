...
`reward_pool_output`. The first four are one row per epoch, `reward_seed_failure`
is up to one row per snapshot type per epoch, and the last two are roughly one
