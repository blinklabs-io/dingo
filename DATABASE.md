...
| `pparams` | `id`, `cbor`, `added_slot`, `epoch`, `era_id` | PK `id`; index `added_slot` | CBOR protocol parameters. Query by `epoch <= ?` and matching `era_id`. A Mithril ledger-state import that persists delayed reward bases writes both the snapshot epoch's current parameters and the preceding epoch's distinct previous parameters in one metadata transaction. The previous payload is validated against that epoch's actual era; a required missing or incompatible payload fails the import before either row commits. |
