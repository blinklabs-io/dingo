...
// TestTieredCborCacheSeedBlockCbor verifies that SeedBlockCbor inserts a block
// into the Tier-2 LRU so a subsequent offset resolution against it hits the
// cache. This is the path the Leios apply code uses after committing an
// endorser-block blob in its own transaction (invisible to the shared
// transaction's snapshot), so an intra-batch validation read resolves the
// endorser-block CBOR from cache instead of the blob store.
func TestTieredCborCacheSeedBlockCbor(t *testing.T) {
	config := CborCacheConfig{
		HotUtxoEntries:  100,
		HotTxEntries:    100,
		HotTxMaxBytes:   1024 * 1024,
	...
	var hash [32]byte
	copy(hash[:], []byte("seed-block-hash-0000000000000000"))
	slot := uint64(3017634)
	// A payload whose bytes [1,3) are a known CBOR slice, mimicking a DOFF
	// reference into the seeded block.
	raw := []byte{0x00, 0x82, 0x03, 0x04, 0x00}

	// Not present before seeding.
	if _, ok := cache.blockLRU.Get(slot, hash); ok {
		t.Fatal("block should not be present before SeedBlockCbor")
	}

	cache.SeedBlockCbor(slot, hash, raw)

	cached, ok := cache.blockLRU.Get(slot, hash)
	require.True(t, ok, "block should be present after SeedBlockCbor")
	assert.Equal(t, raw, cached.RawBytes)
	// The offset extraction that a DOFF resolution would perform returns the
	// exact slice, proving a resolution against this block would hit Tier 2.
	assert.Equal(t, []byte{0x82, 0x03}, cached.Extract(1, 2))
}
