...
// and transaction.go destructure map values of this exact type via its
// unexported ref/slot fields, and the merge logic has no dialect variance to
// consolidate. See internal/rewardstate.CredentialSlotRef for the
// equivalent shared type used by the query helpers below that do carry real
// (dialect-bearing) logic.
...
// for the shared SQL.
func (d *MetadataStoreMysql) RebuildRewardLiveStake(
	slot uint64,
	txn types.Txn,
) error {
	...
	rebuild := func(tx *gorm.DB) error {
		return rewardstate.RebuildLiveStake(tx, slot, "")
	}
	if txn != nil {
		return rebuild(db)
	}
...
func (d *MetadataStoreMysql) RewardLiveStakeNeedsBackfill(
	txn types.Txn,
) (bool, error) {
...
func (d *MetadataStoreMysql) StaleConsensusStakeSnapshotsExist(
	txn types.Txn,
) (bool, error) {
	db, err := d.resolveReadDB(txn)
	if err != nil {
