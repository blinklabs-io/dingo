...
// database parameter limits (e.g. MySQL's max_allowed_packet).
const importUtxoBatchSize = 500

// importAssetBatchSize limits each asset INSERT to stay within
// MySQL's parameter limits. Each Asset has ~7 columns,
...
func (d *MetadataStoreMysql) ImportAccount(
	...
	if err != nil {
		return fmt.Errorf(
			"resolve db for import account: %w", err,
		)
	}
	result := db.Clauses(clause.OnConflict{
...
func (d *MetadataStoreMysql) ImportPool(
	...
	if err != nil {
		return fmt.Errorf(
			"resolve db for import pool: %w", err,
		)
	}
	...
	// On conflict, GORM may not populate pool.ID (MySQL
	...
	// On conflict, GORM may not populate reg.ID (MySQL
...
func (d *MetadataStoreMysql) ImportDrep(
	...
	if err != nil {
		return fmt.Errorf(
			"resolve db for import drep: %w", err,
		)
	}
...
func (d *MetadataStoreMysql) GetImportCheckpoint(
	...
	if err != nil {
		return nil, fmt.Errorf(
			"resolve db for get import checkpoint: %w", err,
		)
	}
...
func (d *MetadataStoreMysql) SetImportCheckpoint(
	...
	if err != nil {
		return fmt.Errorf(
			"resolve db for set import checkpoint: %w", err,
		)
	}
	result := db.Clauses(clause.OnConflict{
