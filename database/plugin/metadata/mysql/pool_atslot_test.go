...
// TestGetEpochBoundaryRewardStakeInputsUseHistoricalExpirationMysql exercises
// the CIP-0163 gated reward-input path on mysql. With the gate on it must
...
func TestGetEpochBoundaryRewardStakeInputsUseHistoricalExpirationMysql(t *testing.T) {
	store := newTestMysqlStore(t)
	// Registered via t.Cleanup (not a plain defer) and ahead of the data
	// cleanup below so LIFO ordering runs the data cleanup BEFORE the
	// connection closes: t.Cleanup callbacks run after every plain defer
...
// TestEpochBoundaryStakeSemanticsMysql exercises the epoch-boundary (SNAP)
// reward cut and the zero-floored reward reconstruction on mysql. Both are
...
// type is the backend's native integer type (UNSIGNED here).
func TestEpochBoundaryStakeSemanticsMysql(t *testing.T) {
	store := newTestMysqlStore(t)
	t.Cleanup(func() { _ = store.Close() })
	db := store.DB()
