...
func cleanImportedAccountTables(store *MetadataStorePostgres) {
	db := store.DB()
...
// TestPostgresRollbackKeepsImportedAccountWithoutRegistrationHistory mirrors the
...
func TestPostgresRollbackKeepsImportedAccountWithoutRegistrationHistory(
	t *testing.T,
) {
	pgStore := newTestPostgresStore(t)
	defer pgStore.Close() //nolint:errcheck
	cleanImportedAccountTables(pgStore)
	...
	require.NoError(t, pgStore.ImportAccount(account, nil))

	// A later re-delegation: a plain delegation certificate, not a registration.
	require.NoError(t, createTestTransactionPg(pgStore.DB(), 1, redelegationSlot))
	...
	require.NoError(t, pgStore.DB().Create(&cert).Error)
	require.NoError(t, pgStore.DB().Create(&models.StakeDelegation{
		StakingKey:    stakeKey,
		PoolKeyHash:   laterPool,
		AddedSlot:     redelegationSlot,
		CertificateID: cert.ID,
	}).Error)
	require.NoError(t, pgStore.DB().Model(&models.Account{}).
		Where("id = ?", account.ID).
	...
	require.NoError(t, pgStore.RestoreAccountStateAtSlot(rollbackSlot, nil))
...
// TestPostgresDeleteAccountRewardsAfterSlotToleratesMissingAccount mirrors the
...
func TestPostgresDeleteAccountRewardsAfterSlotToleratesMissingAccount(
	t *testing.T,
) {
	pgStore := newTestPostgresStore(t)
	defer pgStore.Close() //nolint:errcheck
	...
	require.NoError(t, pgStore.DB().Create(&models.AccountRewardDelta{
		StakingKey:     stakeKey,
		CredentialTag:  0,
		TxHash:         txHash,
	...
	require.NoError(
		t,
		pgStore.DeleteAccountRewardsAfterSlot(100, nil),
		"rollback must complete despite a reward delta whose account row is missing",
	)

	var remaining int64
	require.NoError(t, pgStore.DB().Model(&models.AccountRewardDelta{}).
		Where("staking_key = ?", stakeKey).
