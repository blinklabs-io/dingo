...
// PostgreSQL uses a single connection pool that handles
// concurrent reads natively, so this delegates to resolveDB.
// This method exists for API consistency with the SQLite
// backend, which uses a separate read pool for WAL mode.
func (d *MetadataStorePostgres) resolveReadDB(
	txn types.Txn,
) (*gorm.DB, error) {
...
func (d *MetadataStorePostgres) GetTransactionsByMetadataLabel(
	label uint64,
	limit int,
	offset int,
	descending bool,
...
func (d *MetadataStorePostgres) CountTransactionsByMetadataLabel(
	label uint64,
	txn types.Txn,
) (int, error) {
	db, err := d.resolveDB(txn)
...
func insertMissingDeregistrationAccount(
	...
	// Reload with a locking read so the subsequent deregistration updates the
	// existing row by ID without replacing its delegation fields with the
	// placeholder's zero values.
...
func (d *MetadataStorePostgres) DeleteAddressTransactionsAfterSlot(
	slot uint64,
	txn types.Txn,
) error {
	db, err := d.resolveDB(txn)
...
func (d *MetadataStorePostgres) DeleteTransactionMetadataLabelsAfterSlot(
	slot uint64,
	txn types.Txn,
) error {
	db, err := d.resolveDB(txn)
