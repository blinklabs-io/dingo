...
func (d *MetadataStorePostgres) SavePoolStakeSnapshot(
...
func (d *MetadataStorePostgres) SavePoolStakeSnapshots(
...
func (d *MetadataStorePostgres) GetPoolStakeSnapshot(
	epoch uint64,
	snapshotType string,
	poolKeyHash []byte,
	txn types.Txn,
...
func (d *MetadataStorePostgres) GetPoolStakeSnapshotsByEpoch(
	epoch uint64,
	snapshotType string,
	txn types.Txn,
) ([]*models.PoolStakeSnapshot, error) {
...
func (d *MetadataStorePostgres) GetTotalActiveStake(
	epoch uint64,
	snapshotType string,
	txn types.Txn,
) (uint64, error) {
	...
	result := db.Model(&models.PoolStakeSnapshot{}).
		...
		Select("COALESCE(SUM(CAST(total_stake AS BIGINT)), 0)").
		Scan(&total)
	if result.Error != nil {
...
func (d *MetadataStorePostgres) SaveEpochSummary(
...
func (d *MetadataStorePostgres) GetEpochSummary(
	epoch uint64,
	txn types.Txn,
...
func (d *MetadataStorePostgres) GetLatestEpochSummary(
	txn types.Txn,
) (*models.EpochSummary, error) {
	var summary models.EpochSummary
	db, err := d.resolveDB(txn)
