...
func (d *MetadataStorePostgres) AddAccountRewardByCredential(
	...
	// Normalize the discriminator to a non-NULL empty value when absent so
	// the unique index and the OnConflict/pre-check guards stay effective
	// (a NULL is treated as distinct under SQLite semantics).
	if sourceHash == nil {
		sourceHash = []byte{}
	}
...
func batchFetchCerts(
	...
	if len(refs) > postgresBatchChunkSize {
		for chunk := range slices.Chunk(refs, postgresBatchChunkSize) {
...
func (d *MetadataStorePostgres) RestoreAccountStateAtSlot(
	...
	if txn == nil {
		return d.DB().Transaction(func(tx *gorm.DB) error {
			return d.RestoreAccountStateAtSlot(slot, newPostgresTxn(tx))
		})
	}
	db, err := d.resolveDB(txn)
	if err != nil {
