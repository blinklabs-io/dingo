...
// account/UTxO row in the database, not just this test's rows, so this test
// is intentionally not run in parallel (it would race any other test
// touching reward_live_stake concurrently on the shared postgres container)
// and filters its post-rebuild assertion down to only the staking keys it
// seeded, rather than asserting the whole table's contents.
