...
func (d *MetadataStorePostgres) AddEndorserTransactions(
	hashes [][]byte,
	rbSlot uint64,
	txn types.Txn,
) error {
...
func (d *MetadataStorePostgres) FilterEndorserTransactions(
	hashes [][]byte,
	txn types.Txn,
) ([][]byte, error) {
	if len(hashes) == 0 {
...
func (d *MetadataStorePostgres) UtxoSpenders(
	producerHashes [][]byte,
	txn types.Txn,
) ([][]byte, error) {
...
func (d *MetadataStorePostgres) RevokeEndorserTransactions(
	hashes [][]byte,
	txn types.Txn,
) error {
	if len(hashes) == 0 {
...
func (d *MetadataStorePostgres) DeleteEndorserTransactionsAfterSlot(
	slot uint64,
	txn types.Txn,
) error {
	db, err := d.resolveDB(txn)
