...
func TestCountPoolBlocksInSlotRangeTotalIncludesUnrequestedPoolsPostgres(
	t *testing.T,
) {
	store := newTestPostgresStore(t)
	db := store.DB()
...
func TestRewardLiveStakeRebuildAggregatesCurrentStatePostgres(t *testing.T) {
	store := newTestPostgresStore(t)
	db := store.DB()
...
func TestRewardLiveStakeTracksRewardChangesPostgres(t *testing.T) {
	store := newTestPostgresStore(t)
	db := store.DB()
...
func TestRewardStakeInputsAtSlotComparesDelegationOrderPostgres(t *testing.T) {
	store := newTestPostgresStore(t)
	db := store.DB()
