...
// test builds the pre-v21 row shape by hand (see below), and account.go's
...
// TestAccountRewardReconciledAmountBackfillRecoversPreV21Reconciliation pins
// the v22 migration (dingo #4529) that backfills reconciled_amount for
// ReconcileAccountRewardBalance rows written before v21 added that column.
//
// v21 (account-reward-delta-reconciled-amount) added the column but never
// backfilled it, so on an upgraded database every pre-v21 reconciliation row
...
//
// This test builds that exact pre-v21 database shape by hand (raw SQL,
// migrated only through v21), proves the bug reproduces, runs the v22
// backfill, and proves both the column and the historical read are now
// correct.
func TestAccountRewardReconciledAmountBackfillRecoversPreV21Reconciliation(
	t *testing.T,
) {
	t.Parallel()
	...
	// v1-v21: the schema carries reconciled_amount (v21's own ADD COLUMN),
	// but nothing has backfilled it yet -- exactly an upgraded pre-v22
	// database.
	runTo(registry[:21])
	...
	// The pre-v21 ReconcileAccountRewardBalance write for a
	// TrustCanonicalWithdrawalOnRewardMismatch correction at slot 150: a
	// canonical peer proved the true balance was 5_000_014, not dingo's own
	// 5_000_000. reconciled_amount did not exist yet, so it is absent here
	// (NULL once v21's ADD COLUMN applies) -- amount and previous_reward
	...
	// Before the v22 backfill: reconciled_amount is NULL, so a boundary
	...
	// (and does, when checked against registry[:21] alone) hold.
