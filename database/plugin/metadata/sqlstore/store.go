...
// the commit barrier it holds while fixing its two read views. The read pool
// is small (five connections by default) and an API request can hold its
// read transaction for the whole of a streamed response, so BeginTx there can
// block for an unbounded time; blocking inside the barrier would stall every
// read-write transaction in the process, block application included.
