...
func TestSQLiteRegistry(t *testing.T) {
	...
	require.Equal(t, 18, registry[17].Version)
	require.Equal(t, rewardSnapshotExcludedStakeSchemaRelease, registry[17].Name)
	...
	require.Equal(
		t,
		accountRewardReconciledAmountBackfillRelease,
		registry[20].Name,
	)
	// No schema change; the release exists solely to backfill reconciled_amount
	// for rows v20 could not have populated.
	require.Empty(t, registry[20].SQL["sqlite"].Expand)
	require.NotNil(t, registry[20].Backfill)
	require.Equal(t, "1", registry[20].BackfillRevision)
}
...
// translated to something else -- or a foreign key that did not survive the
// rewrite, leaving rows behind after their utxo is rolled back -- would change
// stake on those backends only.
func TestPointerStakeMigrationTranslatesForProviders(t *testing.T) {
	t.Parallel()

	postgres, err := PostgresRegistry()
	require.NoError(t, err)
	postgresSQL := strings.Join(postgres[12].SQL["postgres"].Expand, "\n")
	...
	require.NoError(t, err)
	mysqlSQL := strings.Join(mysql[12].SQL["mysql"].Expand, "\n")
