	...
	if ret := tx.CollateralReturn(); ret != nil {
		...
		if retAmount != nil && retAmount.Sign() > 0 {
			if !retAmount.IsUint64() || retAmount.Uint64() > sum {
				// A return larger than the resolved inputs means part of the
				// input set was unresolvable; the consumed balance cannot go
				// negative, so report an unresolved lower bound of zero.
				return 0, false, nil
			}
