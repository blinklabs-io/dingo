...
// conversion.
//
// CreditLiveStakeDelta must run inside an active transaction: it takes a
// SELECT ... FOR UPDATE row lock to read the current total_stake for the
// overflow check, then applies the increment in a separate UPDATE. Outside a
// transaction the row lock is released when the SELECT autocommits, so a
// concurrent writer could invalidate the overflow check before the UPDATE
// lands. All callers satisfy this via a caller-supplied txn or db.Transaction.
//
...
func ValidateLiveStakeSourceCredentials(db *gorm.DB) error {
	...
	if err := db.Model(&models.Account{}).
		Where("credential_tag > ? OR LENGTH(staking_key) != ?", 1, 28).
...
// LiveStakeNeedsBackfill reports whether the reward_live_stake aggregate
// needs a one-time RebuildLiveStake pass: true when the account table
// already has rows (this database has synced past genesis) but the
// reward_live_stake aggregate is completely empty. This detects an upgraded
// database that predates the aggregate and never received the backfill (see
// #1959) without misfiring on a legitimately fresh, empty database, where
// both tables are empty and there is nothing to rebuild yet.
func LiveStakeNeedsBackfill(db *gorm.DB) (bool, error) {
	var accountCount int64
	if err := db.Model(&models.Account{}).Count(&accountCount).Error; err != nil {
		return false, fmt.Errorf("counting accounts: %w", err)
	}
	if accountCount == 0 {
		return false, nil
	}
	var liveStakeCount int64
	if err := db.Model(&models.RewardLiveStake{}).
		Count(&liveStakeCount).Error; err != nil {
		return false, fmt.Errorf("counting reward live stake: %w", err)
	}
	return liveStakeCount == 0, nil
}
