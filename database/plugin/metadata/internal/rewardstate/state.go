...
// rewardSaveBatchSize bounds the rows per multi-row INSERT for the reward
// snapshot/output saves. A single Create binds rows*columns parameters; the
...
// DedupePoolKeyHashes returns poolKeyHashes with duplicate entries removed,
...
// StakeInputsAtSlot returns positive per-account delegated stake for pools
// from the live reward stake aggregate. chunkSize bounds how many pool key
// hashes are placed in a single IN (?) clause per query; backends differ
// here (sqlite's SQLITE_MAX_VARIABLE_NUMBER is much lower than mysql's or
// postgres's bind-variable ceiling), so callers pass their own
// backend-appropriate chunk size.
func StakeInputsAtSlot(
	...
	// total_stake is stored as decimal-string TEXT (types.Uint64), so the
	// ordering comparison below needs an explicit cast to the backend's
	// native integer type; see IntegerCastType.
	query := fmt.Sprintf(`
		SELECT rls.*
...
// When txn is non-nil, db is used as-is (the caller already holds an
// external transaction); otherwise the deletes are wrapped in their own
// transaction.
func DeleteInputsForEpoch(
	db *gorm.DB,
	epoch uint64,
	txn types.Txn,
) error {
...
// DeleteOutputsForEpoch deletes reward-calculation output rows for an
// epoch. When txn is non-nil, db is used as-is; otherwise the deletes are
// wrapped in their own transaction.
func DeleteOutputsForEpoch(
	db *gorm.DB,
	epoch uint64,
	txn types.Txn,
) error {
	deleteRows := func(tx *gorm.DB) error {
		if err := tx.Where("epoch = ?", epoch).
			Delete(&models.RewardPoolOutput{}).Error; err != nil {
			return fmt.Errorf("delete reward pool outputs for epoch %d: %w", epoch, err)
		}
		if err := tx.Where("epoch = ?", epoch).
			Delete(&models.RewardAccountOutput{}).Error; err != nil {
...
func DeleteStateAfterSlot(
	...
	deleteRows := func(tx *gorm.DB) error {
		if err := resetAdaPotsForRolledBackOutputs(tx, slot); err != nil {
			return err
		}
		if err := tx.Where(
			"captured_slot > ?",
		...
		if err := tx.Where(
			...
			return fmt.Errorf("delete reward stake inputs after slot: %w", err)
		}
		if err := tx.Where(
			"captured_slot > ? OR boundary_slot > ?",
			slot,
			...
			return fmt.Errorf("delete reward pool outputs after slot: %w", err)
		}
		if err := tx.Where(
			"captured_slot > ? OR boundary_slot > ?",
			slot,
			...
			return fmt.Errorf("delete reward account outputs after slot: %w", err)
		}
		return nil
	}
...
func resetAdaPotsForRolledBackOutputs(tx *gorm.DB, slot uint64) error {
	rolledBackSnapshotEpochs := make(map[uint64]struct{})
	for _, model := range []any{
		...
		if err := tx.Model(model).
			Where("captured_slot > ? OR boundary_slot > ?", slot, slot).
			Distinct().
			Pluck("epoch", &epochs).Error; err != nil {
			return fmt.Errorf("find rolled-back reward output epochs: %w", err)
		}
		for _, epoch := range epochs {
			rolledBackSnapshotEpochs[epoch] = struct{}{}
		}
	}
	if len(rolledBackSnapshotEpochs) == 0 {
		return nil
	}
	potsEpochs := make([]uint64, 0, len(rolledBackSnapshotEpochs))
	for snapshotEpoch := range rolledBackSnapshotEpochs {
		if snapshotEpoch > ^uint64(0)-2 {
			continue
		}
		potsEpochs = append(potsEpochs, snapshotEpoch+2)
	}
	if len(potsEpochs) == 0 {
		return nil
	}
	if err := tx.Model(&models.RewardAdaPots{}).
		Where("epoch IN ?", potsEpochs).
		Update("rewards", types.Uint64(0)).Error; err != nil {
		return fmt.Errorf("reset rolled-back reward ADA pot values: %w", err)
	}
	return nil
}
...
func DeleteStateBeforeEpoch(
	...
	deleteRows := func(tx *gorm.DB) error {
		...
		if err := tx.Where("epoch < ?", epoch).
			Delete(&models.RewardStakeInput{}).Error; err != nil {
			return fmt.Errorf("delete reward stake inputs before epoch: %w", err)
		}
		if err := tx.Where("epoch < ?", epoch).
			Delete(&models.RewardPoolOutput{}).Error; err != nil {
			return fmt.Errorf("delete reward pool outputs before epoch: %w", err)
		}
		if err := tx.Where("epoch < ?", epoch).
			Delete(&models.RewardAccountOutput{}).Error; err != nil {
