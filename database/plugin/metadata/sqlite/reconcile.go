...
func (d *MetadataStoreSqlite) GetActiveAccountCredentials(
	txn types.Txn,
...
func (d *MetadataStoreSqlite) DeactivateAccounts(
	txn types.Txn,
	...
	for _, c := range creds {
		if res := db.
			Model(&models.Account{}).
			Where("credential_tag = ? AND staking_key = ? AND active = ?",
				c.Tag, c.Key, true).
...
// metadata.MetadataStore for the contract.
func (d *MetadataStoreSqlite) DeactivateDreps(
	txn types.Txn,
	creds []models.StakeCredentialRef,
) error {
	if len(creds) == 0 {
	...
	for _, c := range creds {
		if res := db.
			Model(&models.Drep{}).
			Where("credential_tag = ? AND credential = ? AND active = ?",
				c.Tag, c.Key, true).
			Update("active", false); res.Error != nil {
...
func (d *MetadataStoreSqlite) RetirePools(
	txn types.Txn,
	poolKeyHashes [][]byte,
	epoch uint64,
	addedSlot uint64,
	...
	for _, pkh := range poolKeyHashes {
		...
		res := db.Where("pool_key_hash = ?", pkh).First(&pool)
		if errors.Is(res.Error, gorm.ErrRecordNotFound) {
			continue
		}
		if res.Error != nil {
			return fmt.Errorf("RetirePools: lookup pool: %w", res.Error)
		}
		...
		if res := db.Create(&retirement); res.Error != nil {
