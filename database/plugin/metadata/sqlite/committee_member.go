...
// snapshot. On conflict (same cold_cred_hash), the expires_epoch and
// added_slot are updated.
...
func (d *MetadataStoreSqlite) DeleteCommitteeMembersAfterSlot(
	...
	rollback := func(tx *gorm.DB) error {
		// Delete members added after the rollback slot
		if result := tx.Where(
			"added_slot > ?", slot,
		...
		// Clear deleted_slot for members soft-deleted after the rollback slot
