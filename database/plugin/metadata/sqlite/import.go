...
// SQLite's SQLITE_MAX_VARIABLE_NUMBER (default 32766).
// Each UTxO has ~14 columns, so 500 x 14 = 7000 variables.
const importUtxoBatchSize = 500

// importAssetBatchSize limits each asset INSERT to stay within
// SQLite's variable limit. Each Asset has ~7 columns,
...
func (d *MetadataStoreSqlite) ImportAccount(
	...
	if err != nil {
		return fmt.Errorf(
			"ImportAccount: resolve db: %w", err,
		)
	}
	result := db.Clauses(clause.OnConflict{
...
func (d *MetadataStoreSqlite) ImportPool(
	...
	// On conflict, GORM may not populate pool.ID (SQLite
	...
	// On conflict, GORM may not populate reg.ID (SQLite
...
func (d *MetadataStoreSqlite) ImportDrep(
	...
	// Re-fetch ID if upsert didn't populate it (conflict path)
	if drep.ID == 0 {
...
func (d *MetadataStoreSqlite) SetImportCheckpoint(
	...
	if err != nil {
		return fmt.Errorf(
			"SetImportCheckpoint: resolve db: %w",
			err,
		)
	}
	result := db.Clauses(clause.OnConflict{
