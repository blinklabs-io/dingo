...
// Backup/Load format -- S3 has no equivalent point-in-time snapshot
...
// records Restore accumulates per write transaction. s3Txn.Commit applies
...
// memory before ever issuing a PutObject; batching bounds that regardless
// of how large the store being restored is.
const (
...
// S3-native mechanism (S3 has none). database/lifecycle.Snapshot already
...
// S3-backed store this does mean the existing write-pause lasts as long as
...
func (d *BlobStoreS3) Backup(ctx context.Context, w io.Writer) error {
	...
	if _, err := cw.Write(backupMagic[:]); err != nil {
		return fmt.Errorf("s3 backup: write header: %w", err)
	}
	...
	if it == nil {
		return errors.New("s3 backup: blob iterator is nil")
	}
	defer it.Close()
	if err := it.Err(); err != nil {
		return fmt.Errorf("s3 backup: blob iterator: %w", err)
	}
	it.Rewind()
	for it.Valid() {
		item := it.Item()
		...
		if err != nil {
			return fmt.Errorf("s3 backup: read value for key %x: %w", key, err)
		}
		if err := writeRecord(cw, key, value); err != nil {
			return fmt.Errorf("s3 backup: %w", err)
		}
		it.Next()
	}
...
func (d *BlobStoreS3) Restore(ctx context.Context, r io.Reader) error {
	...
	if err != nil {
		return fmt.Errorf("s3 restore: check store is empty: %w", err)
	}
	if !empty {
		return errors.New(
			"s3 restore: store already contains data -- Restore must only be called against a freshly created, empty store",
		)
	}
	...
	if _, err := io.ReadFull(cr, header[:]); err != nil {
		return fmt.Errorf("s3 restore: read header: %w", err)
	}
	if [4]byte(header[:4]) != backupMagic {
		return errors.New("s3 restore: not an s3 backup stream")
	}
	if header[4] != backupVersion {
		return fmt.Errorf(
			"s3 restore: unsupported backup version %d",
			header[4],
		)
	}
	...
	for {
		...
		if err != nil {
			_ = txn.Rollback()
			return fmt.Errorf("s3 restore: %w", err)
		}
		if err := d.Set(txn, key, value); err != nil {
			_ = txn.Rollback()
			return fmt.Errorf("s3 restore: set key %x: %w", key, err)
		}
		batchRecords++
		...
		if batchRecords >= defaultRestoreBatchRecords ||
			...
			if err := flush(); err != nil {
				return fmt.Errorf("s3 restore: commit batch: %w", err)
			}
		}
	}
	if err := flush(); err != nil {
		return fmt.Errorf("s3 restore: commit final batch: %w", err)
	}
	return nil
}

// isEmpty reports whether the store has no keys at all.
func (d *BlobStoreS3) isEmpty(ctx context.Context) (bool, error) {
	if err := ctx.Err(); err != nil {
		return false, err
	}
	txn := d.NewTransaction(false)
