		...
		go func() {
			...
			if db := d.DB(); db != nil {
				...
				if d.closeErr == nil && d.dataDir != "" {
					waitForDirLockRelease(filepath.Join(d.dataDir, "blob"))
				}
			}
			close(d.closeDone)
		}()
...
// badger.DB.Close() call has returned, and the on-disk directory lock is
// free (see waitForDirLockRelease). CloseContext itself may return earlier,
