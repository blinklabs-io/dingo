...
* Noted **no new features:** This patch release focuses on fixes.
...
* Corrected **steadier peer sharing:** Dingo no longer asks peers to request peer sharing from peers that have it turned off or do not support that conversation, so nodes avoid UnknownMiniProtocol errors and connection resets.
