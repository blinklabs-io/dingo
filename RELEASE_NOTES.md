...
- Running under load is more predictable because **race-safety around chain reads** now uses read locks for primary-chain access and current protocol parameter reads and Linux CI now runs the Go race detector.
...
- Operator certificate operations are more consistent because **KES period offsets** are now handled relative to the operator certificate (opcert) and offset edge cases are validated with tests.
