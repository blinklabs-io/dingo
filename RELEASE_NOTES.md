...
- **Sizing and performance benchmarks:** Capacity planning is easier because targeted benchmark suites and sizing reports are now available.
...
- **Peer governance and event fan-out:** Larger peer sets are easier to handle because connection-manager checks and metrics are streamlined, peer governor reconciliation and quota enforcement are optimized, and event publishing now uses snapshot-based fan-out with an async `Close` API.
- **KES period offsets:** Operator certificate operations are more consistent because KES periods are now handled relative to the operator certificate (opcert) and offset edge cases are validated with tests.
...
- **Error handling instead of panics:** Some call paths may now fail explicitly because block metadata marshaling can now return an error for invalid `PrevHash` lengths.
