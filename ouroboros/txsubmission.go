...
func TxsubmissionClientStart(
	...
	if conn.TxSubmission() == nil {
		return fmt.Errorf(
			"TxSubmission protocol not available on connection: %s",
			connId.String(),
		)
	}
	if conn.TxSubmission().Client == nil {
		return fmt.Errorf(
			"TxSubmission client not available on connection: %s",
			connId.String(),
		)
	}
	// Register mempool consumer
	// We don't bother capturing the consumer because we can easily look it up later by connection ID
	consumer := mempool.AddConsumer(connId)
	if consumer == nil {
		return fmt.Errorf(
			"failed to add mempool consumer for connection %s",
			connId.String(),
		)
	}
