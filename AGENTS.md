...
- CI runs both DevNet suites for you on pull requests that touch a consensus-sensitive path (`.github/workflows/devnet.yml`, patterns in `.github/devnet-paths.txt`), and unconditionally on pushes to `main`, `v*` tags, and a nightly schedule. Check the decision for a change set with `./.github/scripts/devnet-path-filter.sh <file-of-changed-paths>`. The stable check name is `devnet-gate`; it is not yet a required status check because the suites are intermittently red on `main` from the fork-tip liveness wedge in #3029. See `docs/BRANCH_PROTECTION.md`. Adding a consensus-sensitive package means adding its pattern to `.github/devnet-paths.txt`; `internal/test/ci/` fails if a pattern matches no real file. CI running these suites does not replace running them locally for consensus-affecting work.
