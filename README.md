...
These same files ship in the release image as `/opt/cardano/config/devnet` (from [docker-cardano-configs](https://github.com/blinklabs-io/docker-cardano-configs)) and are what downstream tooling copies to generate a single-node devnet. Every network under `config/cardano/` exists in both places, so `CARDANO_NETWORK=<net>` serves the same chain whether it resolves to the embedded copy or to the one in the image. `make config-parity` fails when the two drift. It compares against the cardano-configs image tag the Dockerfile pins, so a config change lands here as one commit that bumps the pinned tag and updates `config/cardano/` together; a weekly CI job additionally reports when that repository has moved ahead of the pin.
