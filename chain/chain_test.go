...
// TestRewindPrimaryChainToPointWithHookExcludesConcurrentGrowth covers
// issue #3516's review: a rewind accepted by a dry-run check (ValidateRollback,
// or an equivalent earlier read of tipBlockIndex) can still be invalidated by
// a concurrent AddBlock/AddBlockWithPoint before the real truncation runs,
// since chain growth takes only c.mutex/c.manager.mutex, independent of any
// mutex a caller might hold around its own validate-then-emit step. A hook
// run from inside RewindPrimaryChainToPointWithHook, still holding those
// same locks, cannot be raced by chain growth this way: the concurrent
// AddBlock below must block until the whole rewind (hook included) has
// finished.
func TestRewindPrimaryChainToPointWithHookExcludesConcurrentGrowth(t *testing.T) {
	db := newTestDB(t)
	cm, err := chain.NewManager(db, nil)
	if err != nil {
		t.Fatalf("unexpected error creating chain manager: %s", err)
	}
	// K=3: rewinding from index 6 to index 3 (testBlocks[2]) has forkDepth
	// 3, exactly at the boundary and therefore accepted.
	mustSetLedger(t, cm, 3)
	c := cm.PrimaryChain()
	for _, testBlock := range testBlocks {
		if err := c.AddBlock(testBlock, nil); err != nil {
	...
	// By the time this concurrent AddBlock actually runs (after the
	// rewind below completes), the chain tip has moved back to
	// testBlocks[2], so this must extend from there, not from the
	// pre-rewind tip.
	extension := &MockBlock{
		MockBlockNumber: testBlocks[2].BlockNumber() + 1,
		MockSlot:        testBlocks[2].SlotNumber() + 20,
		MockHash:        testHashPrefix + "00a1",
		MockPrevHash:    testBlocks[2].MockHash,
	}
	...
	go func() {
		<-hookEntered
		// This must block until the rewind (hook included) has released
		// c.mutex/c.manager.mutex, i.e. only after the hook below returns.
		addDone <- c.AddBlock(extension, nil)
	}()
	rewindBlock := testBlocks[2]
	rewindPoint := ocommon.Point{
		Slot: rewindBlock.SlotNumber(),
	...
	// The hook runs while c.mutex/c.manager.mutex are still held, so it
	// must not call back into any chain method that takes them (like
	// Tip()) -- only the plain values the hook itself receives.
	var forkDepthDuringHook uint64
	hookErr := cm.RewindPrimaryChainToPointWithHook(
		rewindPoint,
		func(rollbackBlockIndex uint64, forkDepth uint64) error {
			forkDepthDuringHook = forkDepth
			close(hookEntered)
			// Give the concurrent AddBlock every chance to run if the
			// locks did not actually exclude it.
			testutil.RequireNoReceive(
				t, addDone, 50*time.Millisecond,
				"concurrent AddBlock must not complete during the hook",
			)
			return nil
		},
	)
	if hookErr != nil {
		t.Fatalf("unexpected error rewinding primary chain: %s", hookErr)
	}
	wantForkDepth := uint64(len(testBlocks) - 3) // ID 6 tip down to ID 3
	if forkDepthDuringHook != wantForkDepth {
		t.Fatalf(
			"hook observed fork depth %d, wanted %d",
			forkDepthDuringHook, wantForkDepth,
		)
	}
	addErr := testutil.RequireReceive(
		t, addDone, time.Second,
		"concurrent AddBlock should complete once the rewind is done",
	)
	if addErr != nil {
		t.Fatalf("unexpected error from concurrent AddBlock: %s", addErr)
	}
	tip := c.Tip()
	if tip.Point.Slot != extension.MockSlot ||
		!bytes.Equal(tip.Point.Hash, extension.Hash().Bytes()) {
		t.Fatalf(
			"expected the concurrent block to extend the rewound chain: "+
				"got tip %d.%x",
			tip.Point.Slot, tip.Point.Hash,
		)
	}
