...
If the closure is not yet cached when the block is served, the server waits a bounded window (`LeiosClosureWaitTimeout`, default 3s) for the async client path to populate it — `storeLeiosEndorserBlock` wakes waiters via per-EB channels under `leiosMu`. On success it serves the merged block; on timeout it falls back to serving the raw block (empty transaction segments) plus a warning. Non-certifying blocks and malformed-CBOR cases are served as-is. Two Prometheus metrics capture the outcome: `dingo_leios_ntc_certrb_serves_total{outcome}` counts served CertRBs, with `outcome` one of `merged` (closure cached, spliced without waiting), `merged_after_wait` (closure arrived during the bounded wait), or `raw_unresolved` (wait timed out, raw block served); `dingo_leios_ntc_certrb_closure_wait_seconds{outcome}` records how long the server waited, with `outcome` `resolved` or `timeout`.
