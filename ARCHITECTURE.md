...
If the closure is not yet cached when the block is served, the server waits a bounded window (`LeiosClosureWaitTimeout`, default 3s) for the async client path to populate it — `storeLeiosEndorserBlock` wakes waiters via per-EB channels under `leiosMu`. On success it serves the merged block; on timeout it falls back to serving the raw block (empty transaction segments) and records the incomplete serve (`dingo_leios_ntc_certrb_serves_total{outcome="raw_unresolved"}` plus a warning). Non-certifying blocks and malformed-CBOR cases are served as-is.
