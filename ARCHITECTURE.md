...
| `ledger.connection_recycle_requested` | LedgerState, Ouroboros | Peer connection must be replaced (header/block crypto verification failure, or a leios-fetch protocol that can no longer answer); node wiring translates it to `connmanager.connection_recycle_requested` so `ledger/` does not import `connmanager/` |
