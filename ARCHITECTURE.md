...
`WaitForTx` reports `STAGE_CONFIRMED` only from a forward `ledger.tx`
`TransactionEvent`. The ledger publishes that event after the active-chain
database transaction commits, so the handler does not consume the raw
`ledger.blockfetch` stream that precedes validation and chain selection. A
rollback `TransactionEvent` is ignored and a still-pending reference continues
waiting for a later committed apply. Event delivery is ordered, so a committed
apply already queued ahead of an undo may still produce a confirmation; the
undo is ignored because the UTxO RPC stage stream has no reversal message and
does not retract a confirmation already sent.
