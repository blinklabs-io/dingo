...
Manifest offers claim their `(slot, hash)` occurrence before asynchronous
dispatch, so concurrent offers on different connections cannot queue duplicate
manifest fetches. Claims are released when dispatch is rejected or the fetch
finishes, including errors. The worker rechecks the cache after acquiring its
