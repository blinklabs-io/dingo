...
RPC-only TLS compatibility input among these four providers; Midnight also
uses the pair directly. They are not promoted to Blockfrost, Kupo or Mesh.
...
subtlety is identical in all of them.
...
A Kupo v2.12-compatible chain-index HTTP API, registered as the
`plugin.CapabilityAPIKupo` capability and resolved by `node.go` (and again by
`reinitializeAPIServers` after a live restore or truncate) only in API storage
mode with a non-zero configured port. Like the other three API servers it uses
`internal/apilistener` for its start/stop protocol rather than reimplementing
it, and it serves every route both at the root and below Kupo's `/v1` prefix.

Optional TLS is configured through `plugins.api.kupo.config.tls`; requests do
not require credentials. See "API security" above.

`NodeAdapter` (`adapter.go`) is the only place the HTTP layer reaches the node.
It translates the package-local Kupo contract into `ledger.LedgerState` reads
and coordinated `database` queries, so the handlers hold no database types and
`api/kupo` depends on `ledger` and `database` but on neither `mempool` nor the
Ouroboros protocol packages.

**Pattern set is fixed.** Kupo is normally configured with a pattern set that
determines what it indexes. Dingo indexes every transaction output in API
storage mode, so its installed pattern set is permanently `["*"]`. Pattern
reads report that global pattern, pattern additions are idempotent and return
`["*"]` without rolling the node back or rebuilding an index, and pattern or
match *deletion* is rejected with `400 Bad Request`: honoring it would violate
Dingo's complete-index contract and could remove chain state another API
serves. Kupo patterns remain fully supported as query selectors on the match
routes.

**Checkpoints are a view, not a second store.** `/checkpoints` is a
compatibility view over committed canonical block history rather than an
indexer-resume checkpoint store of its own. It returns a bounded,
exponentially spaced sample across the ledger security window, and the slot
route returns an exact point or its nearest canonical ancestor, which lets
...
consistent view and `If-None-Match` can answer `304 Not Modified` against that
...
- Match queries stream. `nodeMatchIterator` pages the underlying history query
  512 rows at a time through an ordering cursor, hydrates spending-transaction
