...
- **Committee selection**: the voting committee for an epoch is a pure function of the stake distribution as of the end of epoch E-2 — the Mark row for epoch E-1, the same Mark→Set→Go cadence as leader election (`leios.CommitteeSnapshotEpoch` mirrors `praos.StakeSnapshotEpoch`) — and the Dijkstra `CommitteeStakeCoverage` (sigma_c) protocol parameter. Pools are ordered by stake descending (pool key hash ascending on ties) and selected until cumulative stake reaches sigma_c; the 0-based position in that order is the voter's stable `voter_id`. Committees are memoized in memory and recomputed on demand — there is no database table.
