...
Several invariants are checked. A loaded Byron genesis must carry a positive `protocolConsts.k`, and a loaded Shelley genesis must carry a positive `securityParam` and an `activeSlotsCoeff` in `(0, 1]` whose `4k/f` window fits in `uint64` (the ledger substitutes the default below when `3k/f` overflows, and `nonceStabilityWindow` returns 0 when `4k/f` does). Without this, an invalid genesis reached `LedgerState`, whose `securityParamForEra`/`calculateStabilityWindowForEra` (`ledger/state.go`) treat the malformed value as "unavailable" and silently substitute a fixed default (`blockfetchBatchSlotThresholdDefault`, 50000) as both the security parameter and the stability window for chain-selection rollback depth, the consumed-UTxO prune window, and the hard-fork safe zone -- rather than failing the way `internal/node/load.go`'s `loadSecurityParamForConfig` and `ledger/eras/shape.go`'s `StabilityWindowForEra` already do for the same malformed genesis on the `load` replay path and the HFC shape builder. This makes the `serve` genesis-load path consistent with them.
...
The `4k/f` check itself still passes rather than rejects when `securityParam`, `epochLength`, or `activeSlotsCoeff` is zero or absent -- but for `securityParam` and `activeSlotsCoeff` that is now moot in practice, since the security-parameter check above already rejects a non-positive or out-of-range value before this check runs. Only a zero `epochLength` can still reach it.
