...
If the closure is not yet cached when the block is served, the server waits a bounded window for the async client path to populate it — `storeLeiosEndorserBlock` wakes waiters via per-EB channels under `leiosMu`. The window is the same one ledger application uses to gate a ranking block on its endorser block: the Leios pipeline timing's `EndorserBlockWaitSlots` (the certify-by deadline) converted to wall-clock via the Shelley slot length (`LedgerState.EndorserBlockWaitDuration`), not an independent constant; `OuroborosConfig.LeiosClosureWaitTimeout` can override it. On success the server serves the merged block. On timeout it does **not** serve a raw, empty-transaction CertRB (an NtC client would record that point once and never re-fetch it — the exact failure of #2817); instead it returns an error so the chainsync connection is closed and the client retries the same point once the closure is available. Non-certifying blocks are served as-is, and a structurally malformed CertRB (splice failure, not a missing closure) still falls back to raw as a CBOR-safety valve. Two Prometheus metrics capture the outcome: `dingo_leios_ntc_certrb_total{outcome}` counts handled CertRBs, with `outcome` one of `merged` (closure cached, spliced without waiting), `merged_after_wait` (closure arrived during the wait), or `unresolved` (wait timed out, connection closed, block not served); `dingo_leios_ntc_certrb_closure_wait_seconds{outcome}` records how long the server waited, with `outcome` `resolved` or `timeout`.
...
3. Reads the parent ranking block's `LeiosAnnouncement` and selects only the matching eligible Leios endorser-block certificate from the pipeline, or produces and broadcasts a Leios endorser block for the current slot when no CertRB is being forged
