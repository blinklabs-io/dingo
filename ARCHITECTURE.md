...
and operational certificate are always still read from local files: the
agent protocol carries only the KES signing key (and, in serve-key mode, the
operational certificate alongside it). `kesagent.Client` implements the
