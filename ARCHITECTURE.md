...
| `ledger.connection_recycle_requested` | LedgerState, Ouroboros | Peer connection must be replaced (header/block crypto verification failure, or a leios-fetch protocol that can no longer answer); node wiring translates it to `connmanager.connection_recycle_requested` so `ledger/` does not import `connmanager/`. Both hops of that stream are subscribed with `SubscriberBackpressureBlock` (`Node.subscribeLedgerConnectionRecycleTranslation`, `Node.subscribeConnectionRecycleRequests`): each publisher raises exactly one request per connection and then latches, so a detached subscriber would leave the connection unrecycled for the rest of its life |
