...
// rather than a concrete era-specific struct so this keeps working if
// gouroboros ever decodes GetUTxOWhole into a different era-specific type.
//
// Datum and reference script are each folded into a single content hash
// (DatumHash() and ScriptRef().Hash() respectively) rather than
// re-encoded byte-for-byte: both are already content-addressed by
...
func canonicalUTxOEntry(out ledger.TransactionOutput) string {
	...
	// hash of that inline content). One field is enough; there is no
	// case where a real hash mismatch would hide behind a matching
	// inline datum or vice versa.
