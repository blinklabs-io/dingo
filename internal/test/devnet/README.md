...
unavailable to txpump for `TXPUMP_CONFIRMATION_SLOTS` slots: 600 on the
canonical specs, and 75, the blockfetch stability window (`3k/f`), when
`run-tests.sh` or `start.sh` brings the network up with `--accelerated`. That
keeps an output out of coin selection until the block that created it is past
rollback, and lets txpump submit again within the accelerated scenario's
propagation phase. txpump exists to keep the mempool exercised while the
consensus tests run, so block bodies are non-empty and tx-submission / mempool
paths are continuously hit. The image is built from
