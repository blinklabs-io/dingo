...
//
// It first waits for cfg.InitialWait, then polls every cfg.CheckInterval.
// SetupComplete is signaled after the first forged block is observed.
...
func (a *Analyzer) readFile(path string) {
	...
	// Detect file truncation (e.g. log rotation) and reset.
	info, err := f.Stat()
	...
	// Use bufio.Reader + ReadString to track exact byte positions rather
	// than relying on Seek after a buffered scanner.
