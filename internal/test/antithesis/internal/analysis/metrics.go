...
type Metrics struct {
	...
	// ChainTipByNode maps node ID to the slot of the most recent
	// EventChainExtended event, i.e. the node's current chain tip.
...
func (m *Metrics) recordChainExtended(ev *BlockEvent) {
	if ev.Slot > m.ChainTipByNode[ev.NodeID] {
...
func nodeSlotKey(nodeID string, slot uint64) string {
	return nodeID + "\x00" + strconv.FormatUint(slot, 10)
}
