...
func TestMetrics_TracksConfirmedTransactionsAndDuplicateSubmissions(
	t *testing.T,
) {
	m := NewMetrics()
	m.RecordEvent(
		&BlockEvent{Type: EventTxSubmitted, TxID: "tx1", TxType: "payment"},
	)
	m.RecordEvent(
		&BlockEvent{Type: EventTxSubmitted, TxID: "tx1", TxType: "payment"},
	)
	...
	m.RecordEvent(
		&BlockEvent{Type: EventTxSubmitted, TxID: "tx2", TxType: "payment"},
	)
