...
func TestBuildPlutusLockTx_InsufficientInputs(t *testing.T) {
	inputs := []UTxO{{TxHash: sampleHash, Index: 0, Amount: minSendAmount}}
	_, err := BuildPlutusLockTx(
		inputs,
		sampleScriptHash,
		minSendAmount,
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "cannot cover amount")
}
...
func TestBuildPlutusUnlockTx_InsufficientInputs(t *testing.T) {
