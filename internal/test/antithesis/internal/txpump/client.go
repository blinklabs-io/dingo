...
func (c *NodeClient) SubmitTx(eraID uint16, txBytes []byte) error {
	if c.conn == nil {
		return errors.New("SubmitTx: no connection")
	}
	lts := c.conn.LocalTxSubmission()
	if lts == nil {
		return errors.New("SubmitTx: LocalTxSubmission protocol not available")
	}
	if err := lts.Client.SubmitTx(eraID, txBytes); err != nil {
		return fmt.Errorf("SubmitTx era=%d: %w", eraID, err)
	}
	return nil
}
