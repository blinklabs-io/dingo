...
// maxTTL is the TTL value used for all test transactions. Max uint32
// effectively disables TTL expiry.
const maxTTL = 4294967295

// decodeInputs converts a slice of UTxOs into CBOR-ready txBodyInput values,
// validating that each tx hash is exactly 32 bytes of hex.
func decodeInputs(label string, utxos []UTxO) ([]txBodyInput, error) {
	inputs := make([]txBodyInput, 0, len(utxos))
	for _, u := range utxos {
		hashBytes, err := hex.DecodeString(u.TxHash)
		if err != nil {
			return nil, fmt.Errorf("%s: invalid tx hash %q: %w", label, u.TxHash, err)
		}
		if len(hashBytes) != 32 {
			return nil, fmt.Errorf("%s: tx hash %q decoded to %d bytes, want 32", label, u.TxHash, len(hashBytes))
		}
...
// sumInputs returns the total lovelace across the given UTxOs.
func sumInputs(utxos []UTxO) uint64 {
	var total uint64
	for _, u := range utxos {
		total += u.Amount
	}
...
func BuildPayment(p PaymentParams) ([]byte, error) {
	...
	if p.Change > 0 && len(p.ChangeAddr) == 0 {
		return nil, errors.New("payment: non-zero change requires a change address")
	}

	inputs, err := decodeInputs("payment", p.Inputs)
	if err != nil {
		return nil, err
	}
	...
	if p.Change > 0 {
		outputs = append(outputs, txBodyOutput{Address: p.ChangeAddr, Amount: p.Change})
	}
