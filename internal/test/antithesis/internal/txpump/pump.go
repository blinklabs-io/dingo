...
const defaultEpochLength uint64 = 500
...
func (p *Pump) runBatch(
	...
	if len(active) == 0 {
		p.logger.Debug(
			"no configured tx types enabled yet",
			"epoch", epoch,
		)
		return
	}
...
func (p *Pump) submitPayment(client *NodeClient, batchSize int) {
	...
	if err != nil {
		p.logger.Warn("coin selection failed", "required_lovelace", required, "err", err)
		return
	}

	addr := deterministicAddr(inputs[0].TxHash)
	txBytes, err := BuildPayment(PaymentParams{
		Inputs:     inputs,
		ToAddr:     addr,
		ChangeAddr: addr,
	...
	result := p.submitAndLog(client, txBytes, "payment", batchSize, inputs)
	if result.submitted && change > 0 {
		p.wallet.Add(UTxO{TxHash: result.txID, Index: 1, Amount: change})
	}
}
...
func (p *Pump) submitDelegation(client *NodeClient, batchSize int) {
	inputs, change, err := p.wallet.SelectCoins(MinFee)
	if err != nil {
		p.logger.Warn("coin selection failed for delegation", "required_lovelace", MinFee, "err", err)
		return
	}
	...
	result := p.submitAndLog(client, txBytes, "delegation", batchSize, inputs)
	if result.submitted && change > 0 {
		p.wallet.Add(UTxO{TxHash: result.txID, Index: 0, Amount: change})
	}
}
...
func (p *Pump) submitGovernance(client *NodeClient, batchSize int) {
	...
	inputs, change, err := p.wallet.SelectCoins(required)
	if err != nil {
		p.logger.Warn("coin selection failed for governance", "required_lovelace", required, "kind", txKind, "err", err)
		return
	}
	...
	if txKind == "drep_reg" {
		txBytes, buildErr = BuildDRepRegistrationTx(
			inputs, drepKeyHash, drepDeposit, MinFee, changeAddr,
		)
	} else {
	...
	result := p.submitAndLog(client, txBytes, "governance", batchSize, inputs)
	if result.submitted && change > 0 {
		p.wallet.Add(UTxO{TxHash: result.txID, Index: 0, Amount: change})
	}
}
...
func (p *Pump) submitPlutus(client *NodeClient, batchSize int) {
	...
	if txKind == "plutus_lock" {
		required += minSendAmount
	}

	inputs, change, err := p.wallet.SelectCoins(required)
	if err != nil {
		p.logger.Warn("coin selection failed for plutus", "kind", txKind, "required_lovelace", required, "err", err)
		return
	}
	...
	result := p.submitAndLog(client, txBytes, "plutus", batchSize, inputs)
	if result.submitted {
		// For plutus_lock: script output at index 0, change at index 1.
		// For plutus_unlock: change at index 0.
		if txKind == "plutus_lock" {
			p.wallet.Add(UTxO{TxHash: result.txID, Index: 0, Amount: minSendAmount})
		}
		if change > 0 {
			changeIdx := uint32(0)
			if txKind == "plutus_lock" {
			...
			p.wallet.Add(UTxO{TxHash: result.txID, Index: changeIdx, Amount: change})
		}
	}
}

// submitResult holds the outcome of a transaction submission attempt.
type submitResult struct {
	txID      string
	submitted bool
}

// submitAndLog submits a built transaction and handles logging, UTxO return on
// failure, and txlog recording.  Returns the result so callers can handle
// change outputs appropriately.
func (p *Pump) submitAndLog(
	client *NodeClient,
	txBytes []byte,
	txType string,
	batchSize int,
	...
	if submitErr != nil {
		...
		p.logger.Warn(txType+" tx rejected", "tx_id", txID, "err", submitErr)
		p.wallet.ReturnUTxOs(inputs)
	} else {
		entry.Status = "submitted"
		p.logger.Info(txType+" tx submitted", "tx_id", txID)
	}
	...
	return submitResult{txID: txID, submitted: submitErr == nil}
}
