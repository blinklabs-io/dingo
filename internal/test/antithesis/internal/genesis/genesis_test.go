...
func TestParseInsufficientDocs(t *testing.T) {
	...
	require.Error(t, err)
	assert.Contains(t, err.Error(), "expected at least 3")
}
...
func TestLoadActualTestnetYAML(t *testing.T) {
	...
	require.True(t, ok)
	testnetPath := filepath.Join(
		filepath.Dir(thisFile),
		"..",
		"..",
		"testnet.yaml",
	)
	require.FileExists(t, testnetPath)
...
func TestLoadWrapsParseErrorWithPath(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "bad-testnet.yaml")
	require.NoError(t, os.WriteFile(path, []byte("---\nfoo: [\n"), 0o644))

	_, err := Load(path)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "genesis.Load: parse")
