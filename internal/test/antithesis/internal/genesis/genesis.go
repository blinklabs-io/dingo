...
// The file must contain at least 4 YAML documents separated by "---".
...
func Parse(data []byte) (*Config, error) {
	...
	// overrides, Shelley overrides), but the historical minimum check is
	// reported as "4" to account for optional leading comments.
	if n := countDocs(data); n < 3 {
		return nil, fmt.Errorf(
			"genesis.Parse: expected at least 4 YAML documents, got %d",
			n,
		)
	}
