...
func main() {
	...
	if wallet.Len() == 0 {
		logger.Error(
			"wallet is empty; set TXPUMP_GENESIS_UTXO_FILE to seed initial UTxOs",
		)
		os.Exit(1)
	}

	genesisTime := time.Now()
	if cfg.SystemStartUnix > 0 {
		genesisTime = time.Unix(cfg.SystemStartUnix, 0)
	}

	pump := txpump.NewPump(cfg, wallet, logger, txlog, genesisTime)
